...
陣列實例的`find`方法，用於找出第一個符合條件的陣列成員。它的引數是一個回呼函式，所有陣列成員依次執行該回調函式，直到找出第一個返回值為`true`的成員，然後返回該成員。如果沒有符合條件的成員，則返回`undefined`。
...
上面程式碼中，`find`方法的回呼函式可以接受三個引數，依次為當前的值、當前的位置和原陣列。
...
這兩個方法都可以接受第二個引數，用來繫結回呼函式的`this`物件。
